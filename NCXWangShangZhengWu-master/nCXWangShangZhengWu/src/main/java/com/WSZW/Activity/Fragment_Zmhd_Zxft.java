...
import com.WSZW.adapter.GzLm_zxft_listveiw_adapter;
...
public class Fragment_Zmhd_Zxft extends Fragment {
	private ListView lv;
	private String chanIds,pageSize, pageIndex="1", statusIds;
	private GzLm_zxft_listveiw_adapter adapter;
	ProgressBar pbMore;
	TextView tvMore;
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		 ...
		 view = inflater.inflate(R.layout.fragment_zmhd_wsdc, container,
				false);
		context = getActivity();
	...
	private void initView() {
		...
		lv = (ListView) view.findViewById(R.id.lv_wsdc);
	...
	private void loadDatas(int nextpage) {
		...
		getdocuments.getDocuments(context, 1, chanIds, pageSize, pageIndex,
				statusIds);
	}
		...
		protected void handlerLoginInfo(Context paramActivity,
			...
			if (handleResult.getiSuccess().equals("success")) {
				...
				if (lv.getAdapter() == null) {
					...
					adapter = new GzLm_zxft_listveiw_adapter(context,
							title_list);
					addFoot();
