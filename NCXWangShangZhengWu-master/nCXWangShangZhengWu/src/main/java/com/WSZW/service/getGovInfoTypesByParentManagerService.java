 ...
 * 
 */
public abstract class getGovInfoTypesByParentManagerService {
	 ...
	 * 获取信息公开目录一级分类对象集合接口
	 */
	public void getGovInfoTypesByParent(final Context paramActivity, final int paramInt,final String arg0) {
...
//			List<GzLm_secondColumn_bean> list;
			...
			protected Integer doInBackground(Integer[] paramArrayOfInteger) {
				try {
					result=downloadDBB(paramActivity,arg0);
					Log.i(TAG, "获取信息公开目录一级分类对象集合接口---------"+result);
						list=XmlUtil.getFirstColumn(result);
					Log.i(TAG, "其中一条数据一条数据================="+list.get(0).getName().toString());
					Log.i(TAG, "其中一条数据一条数据=======Id========="+list.get(0).getId().toString());
					Constants.list_firstcolumn_bean=list;
...
//						 Constants.gzlm_second_bean=list;
				...
				getGovInfoTypesByParentManagerService.this.handlerLoginInfo(paramActivity,
