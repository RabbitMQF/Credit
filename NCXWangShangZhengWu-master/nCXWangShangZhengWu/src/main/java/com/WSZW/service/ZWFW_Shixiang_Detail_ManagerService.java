...
public abstract class ZWFW_Shixiang_Detail_ManagerService {
	public static final String TAG = "ZWFW_Shixiang_Detail_ManagerService";

	public void get_ZWFW_Shixiang_Detail(final Context paramActivity,
			final int paramInt, final String str) {
		...
		BaseAsyncTask loacl = new BaseAsyncTask(paramActivity, true) {
			...
			ZWFW_ShiXiang_Detail_bean resultGroup1 = new ZWFW_ShiXiang_Detail_bean();
			@Override
			protected Integer doInBackground(Integer[] paramArrayOfInteger) {
				// 调用方法，到服务器上验证，第一个参数是要调用的服务器的方法，
				// 第二个参数是要传的参数，获取返回值
				try {
					result1 = downloadDB("ffxxInfo", url, str);
					result1 = cleanResult(result1);
					resultGroup1 = XmlUtil.get_Zwfw_Shixiang_Detail(result1);
			...
			private String cleanResult(String result) {
				...
				result = result.replace("<br/>", "");
				result = result.replace("</a>", "");
				result = result.replace("doc'>", "doc");
				result = result.replace("10.25.247.236:8081", "117.40.244.134:8081");
			...
			protected void onPostExecute(Integer paramInteger) {
				...
				if (paramInteger == 1) {// 获取到返回的信息
					handleResult.setBean(resultGroup1);
				...
				ZWFW_Shixiang_Detail_ManagerService.this.handlerLoginInfo(
	...
	public void get_ZWFW_Shixiang_xzcf_Detail(final Context paramActivity,
			final int paramInt, final String str) {
		final String url = paramActivity.getResources().getString(
				com.WSZW.Activity.R.string.endpoint);
		BaseAsyncTask loacl = new BaseAsyncTask(paramActivity, true) {
			String result1 = null; // 从服务器获取到的数据
			//List<Zwfw_Shixiang_List_Item> resultGroup1 = new ArrayList<Zwfw_Shixiang_List_Item>();// 将服务器获取到的数据转化为此列表
			ZWFW_ShiXiang_Detai_xzcf_bean resultGroup1 = new ZWFW_ShiXiang_Detai_xzcf_bean();
			@Override
			protected Integer doInBackground(Integer[] paramArrayOfInteger) {
				// 调用方法，到服务器上验证，第一个参数是要调用的服务器的方法，
				// 第二个参数是要传的参数，获取返回值
				try {
					result1 = downloadDB("xzcfFfxxInfo", url, str);
					resultGroup1 = XmlUtil.get_Zwfw_Shixiang_xzcf_Detail(result1);
				} catch (XmlPullParserException e) {
					e.printStackTrace();
				} catch (IOException e) {
					e.printStackTrace();
			...
			protected void onPostExecute(Integer paramInteger) {
				...
				HandleResult handleResult = new HandleResult();
				if (paramInteger == 1) {// 获取到返回的信息
					handleResult.setBean_xzcf(resultGroup1);
					handleResult.setiSuccess("success");
				} else if (paramInteger == -1) {// 链接服务器失败
					Toast.makeText(paramActivity, "链接服务器失败！", Toast.LENGTH_LONG)
							.show();
