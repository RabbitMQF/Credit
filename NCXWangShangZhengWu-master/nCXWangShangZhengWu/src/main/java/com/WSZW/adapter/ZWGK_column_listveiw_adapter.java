...
import com.umeng.update.net.e;

public class ZWGK_column_listveiw_adapter extends BaseAdapter {
	...
	public ZWGK_column_listveiw_adapter(Context context,List<ZWGK_FirstColumn_bean> datas) {
	...
	viewholder.ll.setOnClickListener(new OnClickListener() {
		...
		public void onClick(View v) {
			...
			if(!datas.get(position).getName().equals("信息公开年报")&&!datas.get(position).getName().equals("信息公开指南")){
			infofile.id().put(datas.get(position).getId());
			System.out.println("====首菜单Adapter====="+datas.get(position).getId());
			intent.setClass(context, ZWGK_TwoColumn_mainActivity.class);
